    ...
    pub(crate) async fn connect<R: Runtime>(
            ...
            .0
        {
            ...
            "sqlite" => {
                ...
                // Jewel-ERP patch: pin the SQLite pool to a SINGLE connection.
                // SQLite is single-writer, and the app's money-sensitive writes use
                // cross-call `BEGIN … COMMIT` (see src/db/sqliteRepo.ts withTransaction).
                // With the stock multi-connection pool those statements land on
                // different connections and the transaction never holds — invoices,
                // loan payments, etc. silently fail. One connection serialises every
                // statement onto the same handle so explicit transactions work.
                Ok(Self::Sqlite(
