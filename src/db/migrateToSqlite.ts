...
const businessSpecs = (): TableSpec[] => [
  ...
  { table: "sales_invoices", load: () => db.sales_invoices.toArray(), bools: ["interState", "cancelled"], json: ["paymentDetails"] },
  { table: "sales_items", load: () => db.sales_items.toArray() },
  { table: "urd_items", load: () => db.urd_items.toArray() },
  { table: "loans", load: () => db.loans.toArray(), bools: ["isClosed", "blocked"], json: ["itemsPledged"] },
  ...
  { table: "purchase_payments", load: () => db.purchase_payments.toArray() },
  { table: "purchase_returns", load: () => db.purchase_returns.toArray() },
  { table: "sales_returns", load: () => db.sales_returns.toArray() },
  { table: "sales_return_items", load: () => db.sales_return_items.toArray() },
  { table: "audit_log", load: () => db.audit_log.toArray() },
  { table: "daily_metal_rates", load: () => db.daily_metal_rates.toArray() },
  { table: "cash_vouchers", load: () => db.cash_vouchers.toArray(), bools: ["blocked"] },
  { table: "day_closings", load: () => db.day_closings.toArray() },
  { table: "repairs", load: () => db.repairs.toArray() },
  { table: "repair_history", load: () => db.repair_history.toArray() },
  { table: "refinings", load: () => db.refinings.toArray() },
  { table: "refiners", load: () => db.refiners.toArray(), bools: ["blocked"] },
  { table: "bullion_stock", load: () => db.bullion_stock.toArray() },
  { table: "bullion_movement", load: () => db.bullion_movement.toArray() },
  { table: "inventory_ledger", load: () => db.inventory_ledger.toArray() },
